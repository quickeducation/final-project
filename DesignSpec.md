...
Our proposed solution allows users to create customized problem sets, test themselves on the problem sets, share them with others, and see how they stack up against their peers. More specifically, once users register an account, they can create a customized problem set by allowing users to choose what questions/answers they want to include in a set. After creating a problem set, users can then use their uniquely generated link of the problem set and share it with other users. This ultimately promotes social engagement by providing a simple tool for users to share their own problem sets with others. Furthermore, users that complete a partiuclar problem set can compare themselves against other users that have completed the problem set by viewing a leaderboard of users scores on the problem set. They can also view a global leaderboard that displays top users that have the most 100% completitions on problem sets. Once a user has finished answering a problem set, they can review their missed questions and attempt to complete the problem set again, stimulating their learning as they understand what questions they missed. 
...
- A create problem set error must display as a popup if the user has not entered any text on this page.
- The input fields can only accept 60 or less UTF-8 plaintext characters and displays an error if the user attempts to submit a problem set violating these rules.
...
### Review Missed Q's
![alt text](https://i.imgur.com/atRmhbs.png "Screen 13")
- This page will display corresponding questions and answers that the user answered incorrectly.
- Correct answers are displayed with green check marks adjacent to the answer.
- Incorrect answers are displayed with red x's adjancet to the answer.
- The problem set title corresponding to the current problem set will be displayed at the top.
- The Try Again button will redirect the user back to do the problem set page.
